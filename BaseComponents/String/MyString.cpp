...
void MyString::writeToFile(std::ofstream &ofs) const {
    ofs.write((const char*)&_length, sizeof(_length));
    ofs.write(c_str(),_length);
}
...
MyString operator+(char left, const MyString &rhs) {
    MyString leftStr(left, 0);
    return leftStr+rhs;
}

MyString operator+(const MyString &lhs, char right) {
    MyString rightStr(right, 0);
    return lhs+rightStr;
}

MyString::MyString(char ch, int): MyString(2) {
    _data[0]=ch;
    _data[1] ='\0';
}
